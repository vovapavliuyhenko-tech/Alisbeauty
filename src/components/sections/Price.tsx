...
function PriceTable({ title, rows }: { title: string; rows: Row[] }) {
  return (
    <Reveal>
      <div className="rounded-2xl border border-line bg-bg/40 p-8">
        <h3 className="mb-6 text-2xl text-accent">{title}</h3>
        <ul>
          {rows.map((row, i) => (
            <li
              key={i}
            ...
            >
              <span className="text-text/90">{row.name}</span>
              <span className="whitespace-nowrap font-medium text-accent-2">{row.price}</span>
            </li>
          ))}
        </ul>
      </div>
